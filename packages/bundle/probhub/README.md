...
An installable downstream Bundle that enables ProbHub in a compatible dsh Web profile. It mounts the existing `@greenthree/dsh-host-probhub` bridge and its validation, delivery, and read-only tools. The Web workbench is supplied by the matching downstream dsh Web client; this Bundle does not copy ProbHub Core or UI, open another server, or replace the dsh Web shell.
...
- **Core version compatibility** — the installed ProbHub Core must satisfy the version range declared by `@greenthree/dsh-host-probhub`.
