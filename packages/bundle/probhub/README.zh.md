...
可安装的下游 Bundle，用于在兼容的 dsh Web profile 中启用 ProbHub。它挂载已有的 `@deepseek-ai/dsh-host-probhub` bridge 以及验证、交付和只读工具。Web 工作台由匹配的下游 dsh Web 客户端提供；Bundle 不复制 ProbHub Core 或 UI，不启动第二个服务器，也不替换 dsh Web 外壳。
...
patch 层向 profile 增加 ProbHub Host 行和 `@deepseek-ai/dsh-host-probhub/tools`。安装后，该 profile 中的 Agent 即可使用验证、交付后台任务和有界只读查询。写任务使用当前 Session 工作区和调用者已经获准的 `workspace-write` 策略；正式 `probhub_build` 还要求 `confirm: true` 和标准 DSH approval 通道。
