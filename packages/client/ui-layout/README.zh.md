...
- **规范源编辑是显式操作**：首个编辑器覆盖 `problem.md`；读取和保存都经过 Host 源文件桥接，要求 Session 已处于 `workspace-write`，外部修改会显示 revision 冲突而不会被覆盖。
